...
export async function PUT(req: NextRequest) {
  ...
  for (const r of rows) {
    if (!r.id || !r.montant_mensuel || +r.montant_mensuel <= 0) {
      return NextResponse.json({ error: 'Montant invalide' }, { status: 400 })
    }
    const { error } = await supabase.from('bareme_allocations').update({
      montant_mensuel: +r.montant_mensuel,

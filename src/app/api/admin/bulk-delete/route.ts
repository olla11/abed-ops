...
// DELETE /api/admin/bulk-delete?type=missions|soumissions|payments|notifications&before=2026-01-01
export async function DELETE(req: NextRequest) {
  ...
  const before = req.nextUrl.searchParams.get('before') // date ISO
  ...
  if (type === 'missions') {
    // Ne supprimer que les missions clôturées ou rejetées
    ...
    for (const m of toDelete ?? []) {
      await admin.from('timesheets').delete().or(`prestataire_id.eq.${m.id}`)
      await admin.from('missions').update({ signe_par: null }).eq('id', m.id)
    ...
    count = toDelete?.length ?? 0
    if (count > 0) {
      await admin.from('payments').delete().in('id', toDelete!.map(p => p.id))
    }
    ...
    count = toDelete?.length ?? 0
    if (count > 0) {
      await admin.from('notifications').delete().in('id', toDelete!.map(n => n.id))
    }
  } else {

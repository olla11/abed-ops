...
export async function POST(
  ...
  if (mission.status !== 'reconciliation_de') {
    return NextResponse.json({ error: 'Cette mission n\'est pas en attente d\'autorisation DE' }, { status: 400 })
  }
  ...
  // Autoriser : clôturer définitivement la mission
  await admin.from('missions').update({
    status: 'cloture',
  ...
  const { data: gestionnaires } = await admin
    .from('profiles').select('email, id').in('role', ['de', 'dp', 'caf', 'aaf'])
  ...
  if (emails.length > 0) {
    try {
      ...
      console.error('[Email] Échec envoi rapport autorisation DE:', e)
    }
  }

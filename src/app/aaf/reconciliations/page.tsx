...
export default async function AAFReconciliationsPage() {
  ...
  const { data: missions } = await supabase
    .from('missions')
    .select('id, reference, objet, lieu, date_depart, date_retour, missionnaire:profiles!missions_missionnaire_id_fkey(nom, prenoms)')
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {missions.map((m: any) => (
            <Link key={m.id} href={`/missions/${m.id}`} className="card" style={{
              ...
              <span style={{
                fontSize: 12, fontWeight: 700, color: '#92660b', background: '#fffbeb',

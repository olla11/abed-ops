...
import { getCachedPersonnel, getCachedContrats, getCachedCongesRH, getCachedEvaluations } from '@/lib/cache'
...
export default async function RHDashboardPage() {
  ...
  // Avenant) — seule la racine (sans contrat_parent_id) compte comme
  // échéance distincte, pour ne pas alerter plusieurs fois sur la même personne.
  const contratsExpirants = (contrats ?? []).filter((c: any) =>
    c.statut === 'actif' && !c.contrat_parent_id && c.date_fin && c.date_fin <= in30 && c.date_fin >= today
  )

...
export default function RHDashboardClient({ personnel, contrats, contratsExpirants, congesRecents, congesEnAttenteCount, evaluations, tauxActivite, activeMoisCount, totalActifs }: Props) {
  ...
  // Un contrat/convention peut faire partie d'une chaîne (Offre → Convention/
  // Contrat → Avenant) — seule la racine de la chaîne (sans contrat_parent_id)
  // représente un engagement distinct ; les documents qui s'y rattachent ne
  // doivent pas être comptés en plus pour la même personne.
  const contratsActifs = contrats.filter(c => c.statut === 'actif' && !c.contrat_parent_id).length

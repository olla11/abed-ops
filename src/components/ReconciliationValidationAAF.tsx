...
export default function ReconciliationValidationAAF({
  missionId,
  mission,
}: {
  missionId: string
  ...
  async function handle(action: 'valider' | 'rejeter') {
    ...
    const res = await fetch(`/api/missions/${missionId}/valider-reconciliation-aaf`, {
      method: 'POST',
    ...
    setMsg(action === 'valider' ? 'Réconciliation validée. Transmise à la CAF.' : 'Réconciliation rejetée.')
    setMsgType('ok')
  ...
  return (
    <div className="card" style={{ border: '2px solid var(--abed-amber)', marginTop: 20 }}>
      <h3 style={{ marginBottom: 4, fontSize: 15, color: '#92400e' }}>
        Validation AAF — Réconciliation en attente
      </h3>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <button className="btn" onClick={() => handle('valider')} disabled={!!loading}>
          {loading === 'valider' ? '⏳…' : '✓ Valider & transmettre à la CAF'}
        </button>

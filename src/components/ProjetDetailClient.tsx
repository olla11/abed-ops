      ...
      <div style={{ background: 'white', borderRadius: 16, padding: '32px 28px', width: 420, maxWidth: 'calc(100vw - 32px)', boxShadow: '0 24px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <div style={{ width: 40, height: 40, borderRadius: '50%', background: current.danger ? '#fee2e2' : '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {current.danger ? <AlertTriangle size={20} color="#dc2626" strokeWidth={1.5} /> : <HelpCircle size={20} color="#f59e0b" strokeWidth={1.5} />}
          </div>
          <div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          ...
          {isCreator && (
            <button onClick={() => setDeleteProjetModal(true)} disabled={deleting}
              ...
              {deleting ? '…' : <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Trash2 size={13} color="#dc2626" strokeWidth={1.5} /> Supprimer</span>}
            </button>
          )}
        </div>
              ...
              <div key={act.id}>
                <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 170px 140px 110px 130px 36px', borderBottom: '1px solid #f3f4f6', background: isSelected ? '#f0fdf4' : 'white', position: 'relative' }}
                  ...
                  <div style={{ padding: '11px 12px', display: 'flex', alignItems: 'center', gap: 8, borderRight: '1px solid #f3f4f6', minWidth: 0 }}>
                    ...
                    {(act.commentaires_activites?.length ?? 0) > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 11, color: '#9ca3af', flexShrink: 0 }}><MessageSquare size={11} color="#9ca3af" strokeWidth={1.5} /> {act.commentaires_activites.length}</span>}
                    <button onClick={() => { setShowSubtaskForm(act.id); setSubtaskNom(''); setExpandedSubtasks(e => ({ ...e, [act.id]: true })) }}
                      ...
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#d1d5db' }}><GitBranch size={13} color="currentColor" strokeWidth={1.5} /></button>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <button onClick={() => setDeleteActiviteId(act.id)}
                      ...
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#d1d5db' }}><X size={12} color="currentColor" strokeWidth={1.5} /></button>
                  </div>
                </div>
      ...
      {selectedActivite && (
        <div className="task-panel" style={{ position: 'fixed', top: 60, right: 0, bottom: 0, width: 440, background: 'white', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)', zIndex: 200, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--abed-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <button onClick={() => setSelectedActivite(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', padding: 4, display: 'flex', alignItems: 'center' }}><X size={14} color="currentColor" strokeWidth={1.5} /></button>
          </div>
          ...
          <div style={{ padding: '12px 24px', borderTop: '1px solid var(--abed-border)', display: 'flex', gap: 8 }}>
            ...
            <button onClick={addComment} disabled={sendingComment || !newComment.trim()}
              style={{ background: newComment.trim() ? 'var(--abed-green)' : '#d1d5db', color: 'white', border: 'none', borderRadius: 10, width: 40, cursor: newComment.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Send size={15} color="white" strokeWidth={1.5} /></button>
          </div>
        </div>
      )}
